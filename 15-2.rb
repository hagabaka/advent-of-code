...
def total_field(field, quantities)
  @ingredients.zip(quantities).reduce(0) do |sum, (ingredient, quantity)|
    sum + ingredient[field] * quantity
  end
end

def score(*quantities)
  calories = total_field(:calories, quantities)
  if calories != 500
    return 0
  end
  @fields[0..-2].reduce(1) do |product, field|
    factor = total_field(field, quantities)
