...
DATA.each_line do |instruction|
  _, action, *coordinates = *instruction.match(/(.+) (\d+),(\d+) through (\d+),(\d+)/)
  coordinates.map!(&:to_i)
  (x1, y1, x2, y2) = coordinates
  indexes = (x1..x2).to_a.product((y1..y2).to_a)
  case action
  when 'turn off'
    indexes.each do |index|
      lights[index[0]][index[1]] = false
    end
  when 'turn on'
    indexes.each do |index|
      lights[index[0]][index[1]] = true
    end
  when 'toggle'
    indexes.each do |index|
      lights[index[0]][index[1]] = ! lights[index[0]][index[1]]
    end
  end
end

require 'matrix'
puts(Matrix[*lights].select {|light| light}.count)
