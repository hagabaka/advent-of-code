...
literal_length = lines.map {|line| line.strip.length}.reduce(:+)
encoded_length = lines.map do |literal|
  literal.strip!
  literal.length + literal.scan(/[\\"]/).length + 2
end.reduce(:+)
puts encoded_length - literal_length
__END__
