require 'set'
people = Set.new
relationship = Hash.new(0)
DATA.read.scan(/^([A-Z][a-z]+) .+ (gain|lose) (\d+) .+ ([A-Z][a-z]+)\.$/) do |name1, sign, number, name2|
  people.add name1
  people.add name2
  modifier = Integer(number)
  modifier = -modifier if sign == 'lose'
  relationship[[name1, name2]] = modifier
end
people.add('me')
scores = people.to_a.permutation.map do |permutation|
  happiness = Hash.new(0)
  permutation << permutation.first
  permutation.each_cons(2) do |a, b|
    happiness[a] += relationship[[a, b]]
    happiness[b] += relationship[[b, a]]
  end
  happiness.values.reduce(:+)
end
puts scores.max
__END__
